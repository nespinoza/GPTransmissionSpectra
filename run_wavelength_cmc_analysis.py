...
if not nopickle:
    ...
    c1lc = np.loadtxt(
        "outputs/" + datafile.split(".")[0] + "/white-light/comps.dat",
        unpack=True,
        usecols=(0),
    )
    data["t"] = t
...
white_light_lcmodel = get_transit_model(
    t.astype("float64"),
    ...
    np.median(posteriors["q1"]),
    np.median(posteriors["q2"]),
    ld_laww,
)
    ...
    if not os.path.exists(
        ...
        for i in range(1, 2):
            ...
            jitter = np.append(
                ...
                np.sqrt(
                    np.exp(
                        posteriors["posterior_samples"]["ljitter"][idx_extract]
                    )
                ),
            )
        ...
        pickle.dump(
            out,
            open(out_folder + "/wbin" + str(wi) + "/BMA_posteriors.pkl", "wb"),
        )
